    ...
    <article className="box-shadow-2 smd:flex smd:items-center smd:gap-7 smd:px-7 smd:py-6 rounded-[28px] bg-white px-5 py-5">
      <div className="smd:w-32 smd:shrink-0">
        <div className="smd:block flex items-start justify-between gap-4">
          <div>
            <h3 className="text-primaryNavy smd:text-[16px] text-[18px] leading-[1.2] font-bold tracking-tight">
              {formatScheduleDate(event.eventDate)}
            </h3>
            <p className="text-gray-6 smd:text-[15px] mt-1 text-[13px] font-semibold">
      ...
      <div className="text-gray-6 smd:mt-0 smd:min-w-0 smd:flex-1 smd:border-l smd:border-gray-2 smd:pl-7 smd:text-[16px] smd:leading-6.5 mt-4 space-y-3 text-[13px] leading-5">
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-gray-8 font-bold break-keep">{event.location}</p>
          </div>
        </div>
      ...
      <Link
        ...
        className="bg-primaryAmber smd:mt-0 smd:h-12 smd:w-28 smd:shrink-0 mt-5 flex h-11 w-full items-center justify-center rounded-xl text-[15px] font-bold text-white shadow-[0_12px_24px_rgba(255,185,0,0.24)] transition hover:brightness-105 active:translate-y-px"
      >
        참여 신청
      </Link>
    </article>
