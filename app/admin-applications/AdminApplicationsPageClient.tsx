"use client";

import { useState } from "react";

import { useAllApplicationsQuery } from "@/hooks/react-query/useAllApplicationsQuery";
import { useApplicationMutation } from "@/hooks/react-query/useApplicationMutation";
import {
  cn,
  formatKoreanDate,
  formatKoreanTime,
  getGenderEmoji,
} from "@/lib/utils";
import { AdminEventGroup } from "@/types/admin-application.type";

export const AdminApplicationsPageClient = () => {
  const { data: adminEventData, isLoading } = useAllApplicationsQuery();
  const { confirm, cancel } = useApplicationMutation();
  const [openEventId, setOpenEventId] = useState<number | null>(null);

  if (isLoading) return <div className="p-6">로딩중...</div>;

  return (
    <div className="mx-auto max-w-md space-y-4 p-4">
      <h1 className="text-lg font-semibold">신청자 관리</h1>

      {adminEventData?.map((event: AdminEventGroup) => {
        const isOpen = openEventId === event.eventId;

        return (
          <div
            key={event.eventId}
            className="box-shadow-2 rounded-2xl border-2"
          >
            <div
              onClick={() => setOpenEventId(isOpen ? null : event.eventId)}
              className={cn(
                "flex cursor-pointer items-center justify-between rounded-2xl bg-green-700 p-4",
                event.eventType === "FUNNECTION" && "bg-primaryNavy"
              )}
            >
              <div>
                <div className="font-semibold text-white">
                  {event.round}회차 · {event.eventType}
                </div>
                <div className="text-gray-2 text-sm">
                  {formatKoreanDate(event.eventDate)}{" "}
                  {formatKoreanTime(event.startTime)}
                </div>
              </div>
              <div className="text-gray-1 text-xs">
                🟡 {event.counts.applied} / 🟢 {event.counts.confirmed}
              </div>
            </div>
            {isOpen && (
              <div className="space-y-3 border-t p-4">
                {event.applications.map((item) => (
                  <div key={item.id} className="rounded-xl border p-3">
                    <div className="flex items-center justify-between">
                      <div className="font-medium">{item.nickname}</div>

                      <div
                        className={`rounded-full px-2 py-1 text-xs ${
                          item.status === "CONFIRMED"
                            ? "bg-green-100 text-green-600"
                            : item.status === "CANCELLED"
                              ? "bg-red-100 text-red-500"
                              : "bg-gray-100 text-gray-500"
                        }`}
                      >
                        {item.status}
                      </div>
                    </div>

                    <div className="mt-1 text-sm text-gray-500">
                      {item.birthYear} / {getGenderEmoji(item.gender)}
                    </div>

                    <div className="text-sm text-gray-500">
                      {item.phoneNumber}
                    </div>

                    {item.memo && (
                      <div className="mt-1 text-sm text-gray-700">
                        {item.memo}
                      </div>
                    )}

                    {item.status === "APPLIED" && (
                      <div className="mt-3 flex gap-2">
                        <button
                          onClick={() => confirm(item.id)}
                          className="flex-1 rounded-lg bg-blue-500 py-2 text-sm text-white"
                        >
                          수락
                        </button>

                        <button
                          onClick={() => cancel(item.id)}
                          className="flex-1 rounded-lg bg-red-400 py-2 text-sm text-white"
                        >
                          거절
                        </button>
                      </div>
                    )}
                  </div>
                ))}
              </div>
            )}
          </div>
        );
      })}
    </div>
  );
};
