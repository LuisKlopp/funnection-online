...
const INTRO_PROFILE_ITEMS = [
  {
    ...
    label: "무엇을 하는 사람인가요?",
    description:
      "1인 개발자로 웹사이트를 직접 만들고, 퍼넥션이라는 오프라인 대화 모임을 기획·운영하고 있습니다. 그리고 보드게임 모임도 운영하고 있어요.",
  },
  {
    icon: Lightbulb,
    label: "왜 이걸 시작했나요?",
    description:
      '"좋은 질문 하나가 사람을 연결할 수 있다"는 걸 직접 경험했고, 억지텐션으로 기 빨리는 모임이 아닌 내 텐션 그대로 자연스럽게 대화할 수 있는 모임의 필요성을 느꼈어요.',
  },
  {
    icon: Wrench,
  ...
  return (
    <section className="smd:px-8 smd:pt-28 smd:pb-18 bg-white px-4 pt-18 pb-8">
      <div className="mx-auto max-w-175">
        <div className="smd:items-center smd:grid-cols-[minmax(0,0.88fr)_minmax(0,1fr)] smd:gap-12 grid gap-8">
          <div className="bg-gray-1 relative overflow-hidden rounded-4xl shadow-[0_28px_70px_rgba(17,24,39,0.12)]">
            ...
            <div className="smd:hidden absolute inset-x-0 bottom-0 z-10 bg-linear-to-t from-black/80 via-black/45 to-transparent px-5 py-6">
              <SectionHeading
                eyebrow="Intro"
                eyebrowClassName="text-white/80"
                title={INTRO_HEADING}
                titleClassName="mt-3 text-white"
              />
            </div>
          </div>
          ...
          <div className="smd:max-w-none min-w-0">
            <SectionHeading
              className="mb-8 hidden smd:block"
              eyebrow="Intro"
              title={INTRO_HEADING}
            />
